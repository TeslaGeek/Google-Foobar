...
# * BFS is used in a loop. There is a difference though in the way we use BFS in both algorithms (Ford-Fulkerson and Dinic).
#
# In Ford-Fulkerson's algorithm, we use BFS to find an augmenting path and send flow across this path. In Dinic’s algorithm, we use BFS to check if more 
# flow is possible and to construct level graph. 
...
# This is the reason it works better than Ford-Fulkerson. In Ford-Fulkerson, we send only flow that is send across the path found by BFS.
...
# I re-used the BFS algorithm in https://www.geeksforgeeks.org/dinics-algorithm-maximum-flow/ and worked on the rest of the solution as below.
# The code below passed all the tests on the Foobar. This is a non trivial challenge for sure and almost took 12 days (out of assigned 14 days) 
# to complete. 
